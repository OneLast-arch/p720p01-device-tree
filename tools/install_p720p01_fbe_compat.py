...
fstab_marker = "P720P01: use the Android-format fstab dedicated to vold"
if fstab_marker not in fscrypt:
    fscrypt = replace_one(
        ...
        f'''        // {fstab_marker}.
        if (!ReadFstabFromFile("/system/etc/p720p01.crypto.fstab", &fstab_default)) {{
            PLOG(ERROR) << "Failed to open P720P01 crypto fstab";
            return false;
        }}''',
        "crypto fstab selection",
    )
    fscrypt_path.write_text(fscrypt)
    print(f"Installed P720P01 vold fstab selection in {fscrypt_path}")
else:
    print(f"P720P01 vold fstab selection already installed in {fscrypt_path}")

decrypt_path = Path(sys.argv[2])
decrypt = decrypt_path.read_text()
unwrap_marker = "P720P01: collect Unisoc KeyMint output from update and finish"
if unwrap_marker not in decrypt:
    ...
    print(f"Installed P720P01 synthetic-password unwrap compatibility in {decrypt_path}")
else:
    print(f"P720P01 synthetic-password unwrap compatibility already installed in {decrypt_path}")
