...
s = target.read_text()
if "WriteP720P01BootloaderMessage" in s:
    print(f"P720P01 BCB reboot handling already installed in {target}")
    raise SystemExit(0)
...
""", """namespace {
constexpr const char* kP720P01MiscDevice = "/dev/block/by-name/misc";

bool WriteP720P01BootloaderMessage(const char* command, const char* recovery) {
...
\tstd::string err;
\tif (!write_bootloader_message_to(boot, kP720P01MiscDevice, &err)) {
\t\tLOGINFO("Unable to write P720P01 BCB: %s\\n", err.c_str());
\t\treturn false;
\t}
\tsync();
\treturn true;
...
\t\tcase rb_system:
\t\t\tUpdate_Intent_File("s");
\t\t\tif (!WriteP720P01BootloaderMessage("", ""))
\t\t\t\treturn -1;
\t\t\tcheck_and_run_script("/system/bin/rebootsystem.sh", "reboot system");
\t\t\treturn property_set(ANDROID_RB_PROPERTY, "reboot,");
""", "system reboot")
...
""", """\t\tcase rb_recovery:
\t\t\tif (!WriteP720P01BootloaderMessage("boot-recovery", "recovery\\n"))
\t\t\t\treturn -1;
\t\t\tcheck_and_run_script("/system/bin/rebootrecovery.sh", "reboot recovery");
\t\t\treturn property_set(ANDROID_RB_PROPERTY, "reboot,");
\t\tcase rb_bootloader:
...
target.write_text(s)
print(f"Installed P720P01 direct BCB reboot handling in {target}")
