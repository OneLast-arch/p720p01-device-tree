...
cpp_marker = "P720P01: do not abort a Keystore2 operation after finish"
if cpp_marker not in cpp:
    ...
    print(f"Installed P720P01 Keystore2 operation lifecycle fix in {cpp_path}")
else:
    print(f"P720P01 Keystore2 operation lifecycle fix already installed in {cpp_path}")

header_path = Path(sys.argv[2])
header = header_path.read_text()
header_marker = "P720P01 finished-operation state"
if header_marker not in header:
    ...
    print(f"Installed P720P01 Keystore2 finished-operation state in {header_path}")
else:
    print(f"P720P01 Keystore2 finished-operation state already installed in {header_path}")

storage_path = Path(sys.argv[3])
storage = storage_path.read_text()
storage_marker = "P720P01: retain an existing blob when KeyMint did not upgrade it"
...
""",
    ...
    print(f"Installed P720P01 upgraded key blob guard in {storage_path}")
else:
    print(f"P720P01 upgraded key blob guard already installed in {storage_path}")


metadata_path = Path(sys.argv[4])
metadata = metadata_path.read_text()
metadata_marker = "P720P01: recovery must not query BootControl for vold checkpoints"
if metadata_marker not in metadata:
    ...
    print(f"Installed P720P01 non-checkpoint data mount in {metadata_path}")
else:
    print(f"P720P01 non-checkpoint data mount already installed in {metadata_path}")


keyutil_path = Path(sys.argv[5])
keyutil = keyutil_path.read_text()
keyutil_marker = "P720P01: create the fscrypt session keyring for recovery mounts"
if keyutil_marker not in keyutil:
    ...
    *device_keyring = keyctl_search(KEY_SPEC_SESSION_KEYRING, "keyring", "fscrypt", 0);
    ...
    print(f"Installed P720P01 fscrypt session keyring fix in {keyutil_path}")
else:
    print(f"P720P01 fscrypt session keyring fix already installed in {keyutil_path}")
